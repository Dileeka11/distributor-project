...
import type { ChequeRecord, Customer, GrnChequeRecord, Settlement, SettlementChequeRecord, Supplier } from '@/types';
...
export default function OutstandingPage() {
  ...
  const load = async () => {
    const [{ data: o }, { data: s }, { data: cq }, { data: gcq }, { data: scq }] = await Promise.all([
  ...
  const chequeRows = [...txnRows, ...settleRows]
    .filter((r) => partyFilter === '' || r.partyId === partyFilter);
    ...
    <div className="fade-in">
      ...
      <div className="card">
        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
          ...
          <span className="chip">{historyRows.length} records</span>
        </div>
        <div style={{ maxHeight: 285, overflow: 'auto' }}>
          <table className="tbl">
            <thead><tr><th>Receipt</th><th>Date</th><th>Party</th><th>Direction</th><th>Mode</th><th className="num">Amount</th><th></th></tr></thead>
            <tbody>
              {historyRows.map((s) => (
                <tr key={s.id}>
                  ...
                  <td className="text-[12px]" style={{ color: 'var(--text-muted)' }}>
                    <span className="inline-flex items-center gap-2">
                      ...
                      {s.mode === 'Cheque' && (s.cheques?.length ?? 0) > 0 && (
                        s.passed ? <Badge kind="green">Passed</Badge> : <Badge kind="amber">Not Passed</Badge>
                      )}
                    </span>
                  </td>
                  ...
                  <td className="num">
                    <div className="flex gap-1.5 justify-end">
                      <Button variant="subtle" size="sm" icon={<Edit2 size={14} />} onClick={() => openEditSettlement(s)} />
                      <Button variant="subtle" size="sm" icon={<Trash2 size={14} />} onClick={() => void deleteSettlement(s)} />
                    </div>
                  </td>
                </tr>
              ))}
...
function SettleModal({ side, rec, editSettlement, outstandingOverride, onClose, onSaved }: {
